...
L'option s'appelle `split_tab`. Elle est à FALSE de base pour ne pas changer les programmes existants avec rtaurgus. Pour l'utiliser dans le package il faudra écrire `split_tab = TRUE`.
...
Par ailleurs, pour voir la différence de secret et comprendre pourquoi il est conseillé d'utiliser split_table dans tous les cas à part une table à 4 variables et 0 hiérarchies vous pouvez regarder la vignette sur TEST_SECRET. On s'attend généralement à ce que l'utilisation du split soit plus rapide et que le secret soit plus lourds. Néanmoins étant donné que modular a du mal avec des tableaux à 4 dimensions, on observe parfois que splitter les tables entrainent un secret moins lourd que ne pas les splitter.
...
Avec ces deux tables on crée alors deux hiérarchies emboitées différentes sur la variable SEX_AGE comme on peut le voir avec les tabulations dans les tableaux. On peut noter que pour ne pas avoir de hiérarchies non emboitées nous sommes obligé de faire au moins 2 tables car on ne peut pas expliquer facilement à l'aide de hiérarchies que :
...
On peut dès lors se poser la question sur le nombre de tableaux qui seront créés par cette méthode et la longueur de chacun. Les explications relatives à ce sujet se trouve dans `split_tab_theorie_fr.Rmd`.

Cette méthode crée des tableaux en dépendant du nombre de noeuds  de chaques variables présentes dans le tableaux.

Dès lors après avoir créé notre liste de tables à l'aide de `reduce_dims`, on pose le secret avec `tab_multimanager` et on reforme à partir de cette liste secrétisé le tableaux de base secrétisé avec `restore_format`.
