...
<img src="tab_SEX_AGE_en.png" alt="base_tab" width="600" height="500">
</div>
...
Therefore, this raises the question of the number of tables that will be created by this method, and the length of each one. Explanations on this subject can be found in `split_tab_theorie.Rmd`.

This method creates arrays depending on the number of nodes for each variable in the array.

So, after creating our table list with `reduce_dims`, we set the secret with `tab_multimanager` and reform the secreted base table from this secreted list with `restore_format`.
