...
params:
  ta_lib:
    ...
    choices: ["file_ini",
              ...
              "Poste_4.1.3"]
  ta_lib_man:
    label: "ou manuellement définir chemin emplacement exécutable Tau-Argus"
    input: text
    value: "?"
  eval_ta:
...
# par défaut (notamment pour build), lit la première ligne d'un fichier
# "tauargus_exe.ini" dans répertoire "vignettes" (un développeur rapatriant le
# code source devra créer ce fichier personnalisé)
dir_poste <- sprintf("D:/%s/TauArgus", Sys.getenv("USERNAME")) # windows only ?
loc_tauargus <- c(
  file_ini    = if (file.exists(f <- "tauargus_exe.ini")) readLines(f)[1],
  AUS_commun  = "Y:/Logiciels/Tau/TauArgus.exe",
  Poste_4.1.7 = file.path(dir_poste, "TauArgus4.1.7b4", "TauArgus.exe"),
  Poste_4.1.6 = file.path(dir_poste, "TauArgus4.1.6b2", "TauArgus.exe"),
  Poste_4.1.4 = file.path(dir_poste, "TauArgus4.1.4"  , "TauArgus.exe"),
  Poste_4.1.3 = file.path(dir_poste, "TauARGUS4.1.3"  , "TauArgus.exe")
)
```
...
loc_tauargus <-
  # sélection manuelle prioritaire sur les raccourcis proposés
...
- `missing_dir` : action à effectuer si un dossier où sera écrit le résultat
  n'existe pas (déclencher un erreur ou le créer)
