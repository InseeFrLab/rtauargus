...
params:
  ta_lib :
    label: "Version de Tau-Argus utilisée pour la vignette"
    value: "Poste_4.1.7"
    input: select
    choices: ["Poste_4.1.7", "Poste_4.1.6", "Poste_4.1.4", "Poste_4.1.3",
              "AUS_4.1.7", "AUS_commun", "Home_4.1.7"]
  eval_ta:
    label: "Évaluer les chunks nécessitant Tau-Argus (lent mais complet)"
    value: TRUE 
  warning:
    label: "Afficher warnings"
    value: FALSE
vignette: >
...
```{r version_ta, include = FALSE}
# pour relier les emplacements exacts aux choix fait dans knit with param
dir_aus <- "V:/PNCDEE/Secret statistique et TAU ARGUS/TauArgus autres versions"
dir_poste <- "D:/XKFZV9/TauArgus"
dir_home <- "E:/TauArgus"
nom_exe <- "TauArgus.exe"
loc_tauargus <- c(
  Poste_4.1.7 = file.path(dir_poste, "TauArgus4.1.7b4", nom_exe),
  Poste_4.1.6 = file.path(dir_poste, "TauArgus4.1.6b2", nom_exe),
  Poste_4.1.4 = file.path(dir_poste, "TauArgus4.1.4"  , nom_exe),
  Poste_4.1.3 = file.path(dir_poste, "TauARGUS4.1.3"  , nom_exe),
  AUS_4.1.7   = file.path(dir_aus,   "TauArgus4.1.7b4", nom_exe),
  AUS_commun  = "Y:/Logiciels/Tau/TauArgus.exe",
  Home_4.1.7  = file.path(dir_home,  "TauArgus4.1.7b4", nom_exe)
)
```
...
Le package _rtauargus_ est une interface **R** pour **&tau;-Argus**. Il permet
de :
...
Ces différentes opérations peuvent être exécutées en une seule fois, ou de
manière modulaire. Elles permettent d'intégrer les tâches réalisées par
...
Pour la démonstration qui va suivre, ce document utilise un jeu de microdonnées
fictif :
...
### Installation de &tau;-Argus

_rtauargus_ étant une interface R pour &tau;-Argus, il est nécessaire que ce
logiciel soit accessible depuis le poste de travail. Dans le cas contraire, son
téléchargement est possible sur la
[page dédiée](http://neon.vb.cbs.nl/casc/tau.htm) du site de l'office
néerlandais de statistiques.

### Configuration de l'emplacement de &tau;-Argus
...
prédéfini. Il est possible de le changer pour toute la durée de la session R.
Exemple :

```{r opt_exe, include = FALSE}
loc_tauargus <- unname(loc_tauargus[params$ta_lib])
...
Les deux derniers arguments utilisent la même syntaxe que le batch &tau;-Argus
(consulter le manuel de référence pour plus d'explications).

#### Exemple minimaliste

```{r rtauargus_ex1, eval = params$eval_ta}
rtauargus(
  microdata        = donnees,
  explanatory_vars = "V1",
...
l'instruction précédente avec :
 ...
 - un paramètre permettant d'afficher le statut du secret ;
 - un paramètre permettant de masquer le journal de &tau;-Argus dans la console.

De plus, les tables produites ne sont plus simplement envoyés dans la console.
...
secret1 <-
  rtauargus(
    microdata          = donnees,
    explanatory_vars   = list("V1", c("V1", "V2")), # 2 tabulations
    safety_rules       = "FREQ(3,10)",
    suppress           = "GH(.,100)", # . remplace le numéro de tabulation
    output_options     = "AS+", # pour afficher le statut
...
> caractères entre la parenthètes et la première virgule sont ignorés et les
...
dès lors que les tableaux à secrétiser ont en commun un certain nombre de
caractéristiques.
...
Pour cela, il faut utiliser les fonctions appelées successivement par
`rtauargus`, à savoir :
...
> _Tous les paramètres de ces fonctions peuvent être utilisés dans la fonction
> `rtauargus()`._


### Fonction `micro_asc_rda`

&tau;-Argus n'accepte en entrée que des microdonnées sous une forme bien
spécifique :

- un fichier texte aux colonnes de longueur fixe (fichier .asc) ;
...
Seuls les données et le nom du fichier asc sont renseignés ici. Comme le nom du
...
> _Toutes les fonctions du package accepetent les emplacements de fichiers
> sous forme de chemin relatif. Par exemple, `asc_filename = "donnees.asc"` avec
> un répertoire de travail `Z:/` est équivalent à l'instruction précédente._

#### Exemple élaboré

On ajoute à l'exemple précédent une variable de poids, les informations sur
une variable hiérarchique et un nombre minimal de décimales à écrire pour les
...
> _La fonction ne vérifie pas l'existence des répertoire oufichiers spécifiés.
> Elle ne fait que générer la séquence d'instructions._
...
On complète l'exemple suivant avec :
...
- l'ajout du statut et la suppression des croisements vides pour les deux
  tableaux.
...
n'étaient pas précisés, ce qui a conduit à la création de fichiers temporaires.
Préciser le nom du fichier .arb n'est pas obligatoire non plus. Ces informations
sont récupérables dans la valeur de retour de la fonction.
...
### Fonction `run_tau_argus`

Maintenant que les instructions ont été générées dans un fichier .arb, on peut
le soummettre à &tau;-Argus en mode batch :

```{r run1, eval = params$eval_ta}
secret2 <- run_tauargus("Z:/donnees.arb")
...
le batch ait été exécuté et se soit déroulé sans erreur.

La fonction `import` est typiquement appelée depuis `run_tauargus` en
paramétrant `import = TRUE`. Elle est aussi utilisable de manière indépendante
(par exemple si on ne souhaite pas relancer un batch qui a mis beaucoup de temps
à s'exécuter).

```{r import, results = "hide", eval = FALSE}
secret2 <- import("Z:/donnees.arb")
#  produit le même résultat que run_tauargus("Z:/donnees.arb", import = TRUE)
```
...
En cas d'impossibilité de l'import pour une tabulation donnée, un data.frame
vide est retourné (avec un message d'avertissement).

#### Attributs des tabulations

À chaque data.frame de la liste est associé un ensemble d'attributs ayant pour
but de garder une trace des spécifications passées à &tau;-Argus.
...
```{r attributs, eval = params$eval_ta}
str(secret2[[2]])
```
...
renseigné. Le nom de l'option est le nom de l'argument d'une fonction précédé de
`rtauargus.` :

> _Par exemple, `rtauargus.decimals` sera la valeur utilisée si l'argument
...
Les options disponibles sont listées ci-dessous avec leur valeur par défaut :
...
```{r opt_reset}
reset_rtauargus_options("rtauargus.response_var", "output_type")

str(rtauargus_options())
```

> _Le préfixe `rtauargus.` avant le nom du paramètre est possible, mais 
> pas obligatoire._
...
Le système d'options du package permet par une ou plusieurs instructions
`options(rtauargus.<opt> = <val> )` de définir des options de manière plus
...
> _Cette dernière façon de procéder est **à déconseiller si la chaîne de
...
- Version de &tau;-Argus utilisée : **`r unname(tauargus_info()["version"])`**

### Reste à faire

- implémenter paramètres `totcode`, `missing`, `codelist`, `request_var`,
  `request_code`, `linked`...
- données tabulées en entrée
- importer d'autres formats ("5", "6" ?)
- ...

### Signaler un problème, suggérer une amélioration

Le package est en cours de construction. Toutes les fonctionnalités de &tau;-
Argus n'ont pas été intégrées (la finalité n'est pas de les intégrer toutes,
seulement les plus fréquemment utilisées).

En attendant un éventuel dépôt sur github, les retours (positifs ou négatifs)
peuvent se faire
<a href="mailto:pierre-yves.berrard@insee.fr?subject=package rtauargus">
